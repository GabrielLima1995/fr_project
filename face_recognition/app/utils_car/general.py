...
def kmeans_get_color():
    pass
    ##BUSCA POR COR PREDOMINANTE KMEANS
    #height, width, d = np.shape(car)
    #data = np.reshape(car, (height*width,3))
...
def percentage2pixel(coordinates, ratio_x, ratio_y, resized_y, resized_x):
    

    [x_min, y_min, x_max, y_max] = [
                int(max(corner_position * ratio_y * resized_y, 10)) if idx % 2 
                else int(corner_position * ratio_x * resized_x)
                for idx, corner_position in enumerate(coordinates[1:])
            ]
